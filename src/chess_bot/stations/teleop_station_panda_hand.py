...
from chess_bot.utils.meshcat_utils import AddMeshcatTriad
...
def AddPandaHand(plant,
    ...
    gripper = parser.AddModelFromFile(
        FindResourceOrThrow(
            "drake/manipulation/models/"
            "franka_description/urdf/hand.urdf"))

    # X_7G = RigidTransform(RollPitchYaw(np.pi / 2.0, 0, roll), [0, 0, 0.09])
    X_7G = RigidTransform(RollPitchYaw(0, 0, roll), [0, 0, 0.00])
    plant.WeldFrames(plant.GetFrameByName("panda_link8", panda_model_instance),
                     plant.GetFrameByName("panda_hand", gripper), X_7G)
    return gripper
...
def MakeChessManipulationStation(time_step=0.002,
    ...
    if add_wsg:
        ...
        AddPandaHand(plant, ModelInstanceIndex(panda_idx))
    if add_board:
    ...
    camera_instance = parser.AddModelFromFile(camerabox_fn, 'camera1')
    ...
    camera_instance = parser.AddModelFromFile(camerabox_fn, 'camera2')
    camera_frame = plant.GetFrameByName('base', camera_instance)
    plant.WeldFrames(plant.world_frame(), camera_frame, X_Camera)
    AddMultibodyTriad(camera_frame, scene_graph, length=0.1, radius=0.005)
    ...
    for i in range(plant.num_model_instances()):
        ...
        if model_instance_name.startswith(panda_prefix):
            ...
            # Wsg controller.
            panda_controller = builder.AddSystem(SchunkWsgPositionController())
            panda_controller.set_name(model_instance_name + "_controller")
            builder.Connect(panda_controller.get_generalized_force_output_port(),
                            plant.get_actuation_input_port(model_instance))
            builder.Connect(plant.get_state_output_port(model_instance),
                            panda_controller.get_state_input_port())
            builder.ExportInput(
                panda_controller.get_desired_position_input_port(),
                model_instance_name + "_position")
            builder.ExportInput(panda_controller.get_force_limit_input_port(),
                                model_instance_name + "_force_limit")
            panda_mbp_state_to_panda_state = builder.AddSystem(
            ...
            builder.Connect(plant.get_state_output_port(model_instance),
                            panda_mbp_state_to_panda_state.get_input_port())
            # builder.Connect(plant.get_force_output_port(model_instance),
            #                 wsg_mbp_force_to_wsg_force.get_input_port())
            builder.ExportOutput(panda_mbp_state_to_panda_state.get_output_port(),
        ...
        elif model_instance_name.startswith(panda_hand_prefix):
